# Master Generative Prompt: Onryl Website Builder

This file captures the production-ready master prompt synthesized for generating Onryl website experiences. Use it as the system-level instruction when prompting generative models to create responsive HTML/CSS/JS assets aligned with Onryl's behavioral, competitive, and compliance requirements.
...
## 3) TRUST BADGE PLACEMENT STRATEGY (DO THIS BY DEFAULT)
...
**Color psychology modes** (switch by `urgency_profile` or inferred from **frs**):
...
## 6) DEFAULT PAGE SKELETON (GENERATE & FILL COPY)

> The agent must output a **complete, responsive page** using this structure, then tailor copy/sections via inputs.
...
## 8) COMPETITIVE UX INSIGHTS (BUILD IN)
...
## 9) CTAs & A/B TESTS (GENERATE VARIANTS)
...
## 10) COMPLIANCE GUARDRAILS (ALWAYS OUTPUT)
...
## 11) DELIVERABLES CHECKLIST (RETURN THESE WITH EVERY BUILD)
...
## ✅ EXAMPLE: GENERATE A HOMEPAGE (DEFAULTS APPLIED)
...
### 🔎 Design logic distilled from research (integrated into the prompt)
